...
import time
import functools
from agent import SFMAgent, TTCAgent, VOAgent
...
def readScenario(fileName, scalex=1., scaley=1.):
    ...
    lines = fp.readlines()
    fp.close()
    for line in lines: # create an agent and add it to the list
        if method=="SFM":
            agents.append(SFMAgent(line.split(','), 1, 10, 5, 2000, 0.08, 1.2e5, 2.4e5, 80, 10)) 
        elif method=="TTC":
            agents.append(TTCAgent(line.split(','), 1, 10, 0.5, 5, 0, 10)) 
        elif method=="VO":
            agents.append(VOAgent(line.split(','), 1, 10, 0)) 
...
def initWorld(canvas):
    ...
    colors = ["#FAA", "#6fa8dc","yellow", "white"]
    for a in agents:
...
# the drawing canvas
canvas = Canvas(win, width=pixelsize, height=pixelsize*world_height/world_width, background="#a4a4a4")
canvas.pack()
initWorld(canvas)
start_time = time.time()
